...
function OwnInSearch(props)
{
    ...
    useEffect(() => {
        ...
        {
            return OwnInUtils.hasPermission(item.auth, userRole)
        }
        ...
        function setNavigation()
        {
            dispatch({
                type : "setNavigation",
                value: OwnInUtils.getFlatNavigation(navigation).filter(item => itemAuthAllowed(item))
            });
        }
...
OwnInSearch.propTypes = {};
OwnInSearch.defaultProps = {
    trigger: (<IconButton className="w-64 h-64"><Icon>search</Icon></IconButton>),
    variant: 'full'// basic, full
};

export default withRouter(React.memo(OwnInSearch));
