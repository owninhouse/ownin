...
import {OwnInUtils, OwnInAnimateGroup} from '@ownin';
...
function OwnInShortcuts(props)
{
    const dispatch = useDispatch();
    const shortcuts = useSelector(({auth}) => auth.user.data.shortcuts);
    const navigationData = useSelector(({ownin}) => ownin.navigation);
    ...
    const shortcutItems = shortcuts ? shortcuts.map(id => OwnInUtils.findById(navigationData, id)) : [];

    useEffect(() => {
        function flattenNavigation()
        {
            setNavigation(OwnInUtils.getFlatNavigation(navigationData));
        }
...
OwnInShortcuts.propTypes = {};
OwnInShortcuts.defaultProps = {
    variant: "horizontal"
};

export default React.memo(OwnInShortcuts);
