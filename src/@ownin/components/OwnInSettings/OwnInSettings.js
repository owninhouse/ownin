...
import {OwnInLayoutConfigs} from '@ownin';
...
function OwnInSettings(props)
{
    const dispatch = useDispatch();
    const user = useSelector(({auth}) => auth.user);
    const themes = useSelector(({ownin}) => ownin.settings.themes);
    const settings = useSelector(({ownin}) => ownin.settings.current);
        ...
        return (
            <FormControl component="fieldset" className={classes.formControl}>
                ...
                <RadioGroup
                    ...
                    onChange={handleChange}
                >
                    {Object.entries(OwnInLayoutConfigs).map(([key, layout]) => (
    ...
    {
        const form = OwnInLayoutConfigs[settings.layout.style].form;
        return getForm(form);
    }
...
export default React.memo(OwnInSettings);
