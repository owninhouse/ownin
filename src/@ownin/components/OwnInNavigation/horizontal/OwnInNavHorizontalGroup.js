...
import OwnInNavHorizontalCollapse from './OwnInNavHorizontalCollapse';
...
function OwnInNavHorizontalGroup(props)
{
                                            ...
                                            <ul className={clsx(classes.children, "pl-0")}>
                                                {
                                                    item.children.map((item) => (
                                                        <React.Fragment key={item.id}>
                                                            ...
                                                            {item.type === 'collapse' && (
                                                                <OwnInNavHorizontalCollapse item={item} nestedLevel={nestedLevel} dense={dense}/>
                                                            )}

                                                            {item.type === 'item' && (
                                                                <OwnInNavHorizontalItem item={item} nestedLevel={nestedLevel} dense={dense}/>
                                                            )}

                                                            {item.type === 'link' && (
                                                                <OwnInNavHorizontalLink item={item} nestedLevel={nestedLevel} dense={dense}/>
                                                            )}
                                                        </React.Fragment>
                                                    ))
                                                }
...
OwnInNavHorizontalGroup.propTypes = {
...
OwnInNavHorizontalGroup.defaultProps = {};

const NavHorizontalGroup = withRouter(React.memo(OwnInNavHorizontalGroup));
