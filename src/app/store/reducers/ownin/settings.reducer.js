...
import OwnInLayoutConfigs from 'app/ownin-layouts/OwnInLayoutConfigs';
...
const settings = function (state = initialState, action) {
    switch ( action.type )
    {
        ...
        {
            const newSettings = _.merge({}, state.current, action.value && action.value.layout && action.value.layout.style ? {layout: {config: OwnInLayoutConfigs[action.value.layout.style].defaults}} : {}, action.value)
        ...
        {
            const newSettings = _.merge({}, state.defaults, action.value && action.value.layout && action.value.layout.style ? {layout: {config: OwnInLayoutConfigs[action.value.layout.style].defaults}} : {}, action.value);
...
{
    const defaultLayoutStyle = (OwnInSettingsConfig.layout && OwnInSettingsConfig.layout.style) ? OwnInSettingsConfig.layout.style : "layout1";
    const layout = {
        style : defaultLayoutStyle,
        config: OwnInLayoutConfigs[defaultLayoutStyle].defaults
    };
